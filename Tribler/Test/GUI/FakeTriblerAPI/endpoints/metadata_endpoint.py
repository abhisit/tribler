...
class MetadataEndpoint(resource.Resource):
    def __init__(self):
        ...
        child_handler_dict = {b"channels": ChannelsEndpoint, b"torrents": TorrentsEndpoint}
...
class ChannelsEndpoint(BaseChannelsEndpoint):
    def __init__(self):
        ...
        child_handler_dict = {b"popular": ChannelsPopularEndpoint}
        for path, child_cls in child_handler_dict.items():
            self.putChild(path, child_cls())
    ...
    def render_GET(self, request):
        ...
        return json.twisted_dumps(
            {"results": channels, "first": first, "last": last, "sort_by": sort_by.decode('utf-8')), "sort_desc": int(sort_asc)}
        )
    ...
    def render_GET(self, request):
        ...
        def update_health():
            if not request.finished:
                ...
                request.write(
                    json.twisted_dumps({"health": {"DHT": {"seeders": torrent.num_seeders, "leechers": torrent.num_leechers}}})
                )
                request.finish()
